...
/**
 * Returns true when the current page is the welcome page.
 * @returns {boolean}
 */
function isWelcome() {
  return window.location.pathname.split('/').filter(Boolean).pop() === 'welcome';
}

/**
 * Returns the code base path, substituting the branch prefix in the hostname
 * when window.hlx.branch is set (e.g. to force assets from the main branch).
 * @returns {string}
 */
function getCodeBasePath() {
  if (window.hlx.branch) {
    const { protocol, hostname } = window.location;
    const [, ...rest] = hostname.split('--');
    if (rest.length) return `${protocol}//${window.hlx.branch}--${rest.join('--')}`;
  }
...
function setup() {
  ...
  if (isWelcome()) window.hlx.branch = 'main';
}
...
async function loadCSS(href) {
  const url = href.startsWith('/') && !href.startsWith('//') ? `${getCodeBasePath()}${href}` : href;
...
function decorateIcon(span, prefix = '', alt = '') {
  ...
  img.src = `${getCodeBasePath()}${prefix}/icons/${iconName}.svg`;
  img.alt = alt;
  img.loading = 'lazy';
  img.width = 16;
  img.height = 16;
        ...
        (async () => {
          try {
            const mod = await import(
              `${getCodeBasePath()}/blocks/${blockName}/${blockName}.js`
            );
            if (mod.default) {
